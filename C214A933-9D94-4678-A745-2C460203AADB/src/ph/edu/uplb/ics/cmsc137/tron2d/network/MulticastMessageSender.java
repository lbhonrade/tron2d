...
public class MulticastMessageSender extends Thread {
	...
	public MulticastMessageSender(TronChannel tChannel,String _data){
		super();
		channel = new WeakReference<TronChannel>(tChannel);
		data = _data;
	}
	...
	public void run(){
		try {
			DatagramPacket packet = new DatagramPacket(data.getBytes(), data.length(), channel.get().mCastAddress, TConstants.connectionPort);
